...
// and the Level Complete panel in the third level
        ...
        if (!globalController.pause)
        {
            if (globalController.hasMoved)
            {
                ...
                else
                {
                    ...
                    highScore.text =
                        "Best time: " +
                        ((int)(globalController.lowestTime3 / 60f)).ToString("00") +
                        ":" +
                        ((int)(globalController.lowestTime3 % 60f)).ToString("00") +
                        ":" +
                        ((int)(globalController.lowestTime3 * 1000f) % 1000).ToString("000");
                     }
                    if (timeSinceMove <= globalController.lowestTime3)
                    {
                        globalController.lowestTime3 = timeSinceMove;
                    }
